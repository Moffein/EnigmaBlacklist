...
namespace EnigmaBlacklist
{
    ...
    public class EnigmaBlacklist : BaseUnityPlugin
    {
        ...
        private void ReadConfig()
        {
            ...
            blacklistString = Config.Bind("Settings", "Blacklist String", "Recycle, Tonic, BossHunter, BossHunterConsumed, GoldGat", "Equipments to blacklist. Separated by comma, case-sensitive. Full list can be found at https://github.com/risk-of-thunder/R2Wiki/wiki/Item-&-Equipment-IDs-and-Names").Value;
        ...
        public void Awake()
        {
            ...
            On.RoR2.Artifacts.EnigmaArtifactManager.OnRunStartGlobal += (orig, run) =>
            {
                ...
                //Add whitelisted equipments to engima
                if (useWhitelist)
                {
                    foreach (EquipmentIndex ei in whitelist)
                    {
                        if (!EnigmaArtifactManager.validEquipment.Contains(ei))
                        {
                            EnigmaArtifactManager.validEquipment.Add(ei);
                        }
                    }
                }
            };
